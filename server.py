...
s.bind(addr)
print("Waiting to authentication....")
hatalideneme = 0

(data,addr) = s.recvfrom(bufferdata) #receive username
data = data.decode("utf-8")
while True:
    kadi = crypt.sifrecoz(data) # verifying
    (data,addr) = s.recvfrom(bufferdata) #receive password
    data = data.decode("utf-8")
    sifre = crypt.sifrecoz(data) # verifying
    if loginkontrol(kadi,sifre) == True:
        print("Authentication is verifying")
